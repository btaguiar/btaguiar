# Bruno Aguiar  

**Marketing Analytics | G4 Educação**  
Transformo métricas em estratégia.  
Atuo na interseção entre **conteúdo criativo, performance digital e análise de dados**, otimizando campanhas com foco em ROI, LTV e CAC.

---
[LinkedIn](https://www.linkedin.com/in/bruno-aguiar-marketing-analytics/)

## Sobre mim  

Da criação à conversão. Do insight à ação.  

Comecei minha carreira desenhando narrativas criativas para marcas. Hoje, aplico o mesmo raciocínio estratégico para **decodificar padrões em dados de marketing**, identificar oportunidades de crescimento e orientar decisões baseadas em evidências.  

Atuo na fronteira entre **marketing e ciência de dados**, unindo visão criativa e pensamento analítico para construir modelos preditivos, testes A/B e análises de performance que conectam investimento a resultado.  

**Diferenciais:**  
- Transição sólida de produção audiovisual para análise quantitativa aplicada ao marketing  
- Capacidade de traduzir dados complexos em narrativas compreensíveis  
- Foco em impacto de negócio e otimização contínua  

---

## Stack Técnico  

| Categoria | Ferramentas |
|------------|-------------|
| **Linguagens** | Python, SQL, JavaScript, Java |
| **Análise e Modelagem** | Pandas, NumPy, Statsmodels, PyMC, Scikit-Learn |
| **Visualização** | Power BI, Seaborn, Matplotlib, Looker Studio |
| **Cloud & Infraestrutura** | AWS (S3, EC2, Lambda), GitHub Actions |
| **Bancos de Dados** | MySQL, SQLite, BigQuery |
| **ETL & Automação** | Python Scripts, API Integrations |

---

## Projetos em Destaque  

**[SCS – Análise de Performance 360°](https://github.com/btaguiar/SCS-Analise-Performance-360)**  
`Python` `SQL` `Matplotlib` `Seaborn`  
Análise multivariada de performance com ROI +1600% e redução de CPA em 40%.

**[BK-DEP – Otimização de Campanha Bancária](https://github.com/btaguiar/BK_DEP_Otimiza-o_de_Convers-o)**  
`Python` `PyMC` `A/B Testing`  
Modelagem bayesiana aplicada à conversão e redução de CPA em 30%.

**MK-SEG – Segmentação Preditiva de Clientes**  
`SQL` `Scikit-Learn` `Power BI`  
Clusterização RFM e análise de propensão de compra. *(Em desenvolvimento)*

---

## Próximos Passos  

- Implementar **modelos bayesianos** (Robyn/PyMC) para otimizar distribuição de budget e ROI  
- Construir **data pipelines** em SQL + Python para conectar campanhas a dashboards analíticos  
- Desenvolver **painéis de performance multicanal** (Meta, Google Ads, LinkedIn) no Power BI / Looker Studio  
- Integrar **modelos preditivos** em Scikit-Learn para estimar LTV e propensão à conversão  
- Publicar estudos e dashboards no portfólio com foco em *Marketing Analytics e Data Storytelling*  

---

## Formação  

**PUCPR** — *Análise e Desenvolvimento de Sistemas*  
fev/2025 – jun/2027 | Foco em análise de dados, SQL e inteligência de negócios  

**FAPCOM** — *Bacharelado em Rádio, TV e Comunicação Digital*  
jan/2014 – nov/2017 | Nota 10,0 | Ênfase em narrativa e comunicação estratégica  

---

## Contato  

Trabalho com empresas que desejam **traduzir dados em estratégias de crescimento**.  
Se você busca alguém que una visão criativa, rigor analítico e foco em resultado, entre em contato:

[LinkedIn](https://www.linkedin.com/in/bruno-aguiar-marketing-analytics/) • [Email](mailto:bruno.aguiarsp@outlook.com) • [Portfólio](https://github.com/btaguiar)

---

MIT License © 2025 Bruno Aguiar
